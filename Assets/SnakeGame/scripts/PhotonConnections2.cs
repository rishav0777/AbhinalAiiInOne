...
public class PhotonConnections2 : MonoBehaviourPunCallbacks
{
    ...
    private void Start()
    {
        if (DataSaver2.Instance.GetMode() == 1) PhotonNetwork.ConnectUsingSettings();
        else StartCoroutine(JoinGame());
    }
    private void Update()
    {
        if (gameFlag)
